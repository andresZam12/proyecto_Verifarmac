...
class InvimaMockDataSource {
  ...
  static const _datos = <String, Map<String, dynamic>>{
    '7702025010015': {
  ...
  Future<MedicineModel?> buscarPorBarcode(String barcode) async {
    await Future.delayed(const Duration(milliseconds: 500));
    final datos = _datos[barcode];
    if (datos == null) return null;
    return MedicineModel.fromJson({...datos, 'id': barcode});
  }

  // Busca por registro sanitario INVIMA
  Future<MedicineModel?> buscarPorRegistro(String registro) async {
    await Future.delayed(const Duration(milliseconds: 500));
    for (final entrada in _datos.entries) {
      if (entrada.value['registro'].toString().contains(registro)) {
  ...
  Future<List<MedicineModel>> buscarPorTexto(String texto) async {
    await Future.delayed(const Duration(milliseconds: 400));
    final query = texto.toLowerCase();
    return _datos.entries
        .where((e) =>
            e.value['nombre'].toString().toLowerCase().contains(query) ||
            e.value['ingrediente'].toString().toLowerCase().contains(query))
